  ...
  >
    <header class="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
      <div class="min-w-0">
        ...
        <div class="mt-4">
          <input
            :value="config.name"
            class="w-full bg-transparent text-[clamp(1.55rem,1.8vw,1.9rem)] font-bold leading-none tracking-[-0.04em] text-slate-800 outline-none placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500"
      ...
      <div class="flex flex-wrap items-center gap-2.5">
        <button
          :class="[
            'inline-flex h-11 cursor-pointer items-center gap-2 rounded-2xl border px-4 text-[13px] font-semibold transition',
            config.enabled
    ...
    <div class="mt-6 grid gap-4 xl:grid-cols-2">
      <label class="space-y-2">
        <span class="text-[13px] font-bold text-slate-700 dark:text-slate-200">CPA 接口地址</span>
      ...
      <label class="space-y-2">
        <span class="text-[13px] font-bold text-slate-700 dark:text-slate-200">密钥</span>
        <div class="flex min-h-14 items-center gap-3 rounded-[22px] border border-white/65 bg-white/72 px-4 shadow-sm transition focus-within:border-blue-300 focus-within:bg-white dark:border-slate-700/70 dark:bg-slate-950/46 dark:focus-within:border-sky-400 dark:focus-within:bg-slate-950/66">
          <el-icon class="text-slate-400 dark:text-slate-500">
            <Key />
          </el-icon>
          ...
          />
          <button
            class="shrink-0 cursor-pointer rounded-full px-2 py-1 text-[11px] font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
            type="button"
      ...
      <label class="space-y-2">
        <span class="text-[13px] font-bold text-slate-700 dark:text-slate-200">CPA 鉴权方式</span>
      ...
      <div class="space-y-2">
        <span class="text-[13px] font-bold text-slate-700 dark:text-slate-200">超时时间（秒）</span>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div class="inline-flex h-14 w-fit items-center overflow-hidden rounded-[22px] border border-white/65 bg-white/72 shadow-sm dark:border-slate-700/70 dark:bg-slate-950/46">
            ...
            <input
              ...
              class="h-full w-20 bg-transparent text-center text-base font-bold tabular-nums text-slate-800 outline-none dark:text-slate-100"
              type="number"
              min="1"
              max="300"
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :key="preset"
              :class="[
                'inline-flex h-10 cursor-pointer items-center rounded-full border px-3 text-[11px] font-semibold transition',
    ...
    <footer class="mt-6 flex flex-wrap gap-2">
      <span class="inline-flex min-h-9 items-center rounded-full border border-blue-200 bg-blue-50 px-3 text-[13px] font-semibold text-blue-600 dark:border-sky-700/70 dark:bg-sky-500/18 dark:text-sky-300">
        {{ authTypeLabel }}
      </span>
      <span class="inline-flex min-h-9 items-center rounded-full border border-slate-200 bg-white/75 px-3 text-[13px] font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300">
        {{ config.timeoutSeconds }} 秒超时
      </span>
      <span class="inline-flex min-h-9 max-w-full items-center rounded-full border border-slate-200 bg-white/75 px-3 text-[13px] font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300">
