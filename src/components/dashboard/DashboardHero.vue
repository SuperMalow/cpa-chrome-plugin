<template>
  <section class="mx-auto mb-5 flex w-full max-w-[1180px] flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
    <div>
      ...
      <p class="mt-2 text-[15px] text-slate-500 dark:text-slate-400">{{ subtitle }}</p>
    </div>

    <div class="flex flex-wrap gap-3">
      <button v-for="action in actions" :key="action.label" :class="[
        'inline-flex h-8 cursor-pointer items-center gap-2 rounded-2xl border px-4 text-[13px] font-semibold transition hover:shadow-[0_10px_24px_rgba(112,136,181,0.1)]',
      ...
      ]" type="button" @click="$emit('action', action)">
        <el-icon>
