<template>
  <div class="min-h-screen bg-gradient-to-b from-slate-100 via-blue-50 to-slate-50 px-4 py-8 md:px-6 md:py-12 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
    ...
    <main class="mx-auto w-full max-w-[1180px] space-y-4">
      <section class="grid gap-4 xl:grid-cols-3">
        <SettingsSummaryCard
          label="CPA 接入数"
          :value="String(settingsStore.totalCount)"
          :note="`${settingsStore.enabledCount} 个已启用`"
          tone="neutral"
        />
        <SettingsSummaryCard
          label="已启用"
          :value="String(settingsStore.enabledCount)"
          note="建议保留一个默认可用接入"
          tone="success"
        />
        <SettingsSummaryCard
          label="鉴权覆盖"
          :value="String(activeAuthCount)"
          :note="authSummaryText"
          tone="accent"
        />
      </section>
      ...
      <section class="rounded-[28px] border border-slate-200/90 bg-white/62 p-4 shadow-[0_24px_64px_rgba(134,154,192,0.16)] backdrop-blur-xl md:p-5 dark:border-slate-800 dark:bg-slate-900/54 dark:shadow-[0_24px_64px_rgba(2,6,23,0.34)]">
        ...
        <div v-else class="mt-5 space-y-4">
          <CpaSettingsCard
            ...
            :index="index"
            @update="handleUpdate"
            @duplicate="settingsStore.duplicateConfig"
            @remove="settingsStore.removeConfig"
          />
        </div>
      </section>
    </main>
